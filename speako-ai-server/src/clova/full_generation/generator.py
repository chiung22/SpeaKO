...
class FullScriptGenerator:
    ...
    def generate_full_script(self, ppt_text, presentation_time, style, extra_requirement="", audience=""):
        """
        ...
        if len(blocks) <= 1:
            ...
            return self._request_raw(ppt_text, presentation_time, style, extra_requirement, None, audience)
        ...
        def build_one(item):
            ...
            slides = self._request_one_slide(
                number, block, _position_label(index, len(blocks)), per_slide_time, style, extra_requirement, audience
            )
    ...
    def _request_one_slide(self, slide_number, block, position, presentation_time, style, extra_requirement, audience=""):
        """
        ...
        for attempt in range(2):
            text = self._call_hcx(
                ...
                self._build_user_prompt(block, presentation_time, style, extra_requirement, position, audience),
            )
    ...
    def _build_user_prompt(self, ppt_text, presentation_time, style, extra_requirement, position, audience=""):
    ...
    def _request_raw(self, ppt_text, presentation_time, style, extra_requirement, valid_slide_numbers, audience=""):
        """브리프 한 덩어리를 여러 슬라이드로 확장하는 경로. 여기서만 TOON이 필요하다."""
        toon_text = self._call_hcx(
            self._MULTI_SLIDE_PROMPT,
            self._build_user_prompt(ppt_text, presentation_time, style, extra_requirement, "", audience),
        )
