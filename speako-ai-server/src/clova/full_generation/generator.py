...
def _thin_source_instruction(slide_number, block=None, role_hint=None):
    ...
    if role_hint:
        ...
        return (
            ...
            "원문에 이름이 여러 개면 동일 인물의 별명과 본명일 수 있습니다 — 인원수나 "
            "이름 사이의 관계를 단정하는 문장('제 옆에는 OO 님이 함께합니다' 등)을 "
            "만들지 말고, 원문에 적힌 그대로만 언급하세요. "
...
_TRANSITION_UNIT = (
    ...
    # "~하시기를/이루기를 기대합니다"
    r"|[^.!?]*기대(?:합니다|하겠습니다)"
    # ── 위 다짐형의 우회 변형들 (2026-08-19 실측: 금지하니 표현만 바꿔 재등장) ──
    # "더 나은 경험을 제공할 것을 약속드립니다"
    r"|[^.!?]*약속드립니다"
    # "여러분의 언어 능력 향상에 큰 도움이 되기를 바랍니다"
    r"|[^.!?]*되기를 바랍니다"
    # "자신감을 높여드리겠습니다", "부담을 덜어드리겠습니다", "도움을 드리겠습니다"
    r"|[^.!?]*(?:높여|덜어|도움을)\s*드리겠습니다"
    # "서비스를 지속적으로 발전시키겠습니다" (발전시켜 나가~의 변형)
    r"|[^.!?]*발전시키겠습니다"
    # "지금 바로 ~를 체험해보세요 / 활용해 보세요 / 누려보세요 / 준비해 보십시오" — CTA 맺음.
    # 동사를 나열하면 끝이 없어서(체험→활용→누려…로 계속 변형됨) '~보세요/보십시오'로 일반화한다.
    # 화면 안내("함께 봐주시기 바랍니다")는 형태가 달라 걸리지 않는다.
    r"|[^.!?]*보(?:세요|십시오)"
    r"|[^.!?]*해\s*보시기\s*바랍니다"
    # "항상 지원하겠습니다 / 함께하겠습니다 / 노력하겠습니다" — 다짐 변형
    r"|[^.!?]*(?:지원|노력|함께)하겠습니다"
)
...
class FullScriptGenerator:
    ...
    _SINGLE_SLIDE_PROMPT = """
        ...
        3. 문장은 짧고 간결하게, 한 문장에 하나의 내용만 담으세요. 숫자와 근거 같은 핵심 정보
           위주로 말하고, '지금 보시는 화면에는 ~가 적혀 있습니다'처럼 화면에 보이는 것을
           묘사하는 문장은 쓰지 마세요 — 청중도 같은 화면을 보고 있습니다.
        4. [추가 요구사항]이 주어지면 반드시 반영하세요.
        5. [반드시 지킬 것 — 위치]의 지시를 그대로 따르세요.
    ...
    _MULTI_SLIDE_PROMPT = """
        ...
        1. 각 슬라이드의 핵심 메시지를 파악해 발표자가 말하듯 작성하세요.
           문장은 짧고 간결하게, 한 문장에 하나의 내용만 담으세요.
