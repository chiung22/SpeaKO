...
2026-08-09 작업분은 **PR로 머지 완료**했습니다(TTS 연결, 피그마 반영 5건, 녹음 상한 재설계,
근거 없는 내용 차단 2건, 단어 목록 비결정성 수정). **pytest 340건 통과, 엔드포인트 19개.**

남은 블로커는 `SPEAKO_API_KEY`(배포 전 필수) 하나입니다. 아래 "API 키 상태" 표를 볼 것.
