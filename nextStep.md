...
# 🔖 작업 재개 지점 (2026-08-10 기준)

> **대화 컨텍스트가 비워진 뒤 이어서 작업할 때 여기부터 읽으세요.**
> 바로 아래 "▶ 다음에 할 일"이 지금 지시받은 작업입니다.

## ▶ 다음에 할 일 — 스프링 로컬 연동 테스트 대기

스프링 담당자와 연동 조건을 전부 합의했다(아래 "스프링 연동 확정 사항"). **시간이 잡히면
서버를 띄우고 대기하는 것이 다음 작업.** 테스트 절차:

1. `uvicorn`으로 서버 기동 (포트 8000) → `GET /` 헬스체크 확인
2. 스프링이 `POST /api/projects`(mode=coaching, script_text)로 프로젝트 생성 → project_id 확보
3. `POST /api/evaluation/audio`(project_id + 짧은 녹음)로 전체 경로 확인
4. 문제가 나면 서버 콘솔 로그를 그대로 스프링 담당자와 공유

2026-08-09 작업분은 **PR #32·#33으로 머지 완료**(TTS 연결, 피그마 반영 5건, 녹음 상한 재설계,
근거 없는 내용 차단, 단어 목록 비결정성, 조각 문장 필터). **pytest 341건 통과, 엔드포인트 18개.**

남은 배포 블로커는 `SPEAKO_API_KEY` 하나. EC2에 올리기 **직전에** 생성해 스프링과 공유한다
