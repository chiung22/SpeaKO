...
# 🔖 작업 재개 지점 (2026-08-07 기준)

> **대화 컨텍스트가 비워진 뒤 이어서 작업할 때 여기부터 읽으세요.**
> 바로 아래 "▶ 다음에 할 일"이 지금 지시받은 작업입니다.

## ▶ 다음에 할 일 — TTS 엔드포인트 연결 + 전체 테스트 (자율 진행 지시받음)

사용자 지시: **"붙이고 모든 테스트 자율로 진행하도록 해."** 확인차 멈추지 말고 끝까지 진행할 것.

**Clova Voice 키가 2026-08-06에 발급돼 `.env`에 들어갔고, 실제 합성 호출까지 성공했습니다**
(`권리` → 7,908 bytes MP3, 0.96초. 안전모드 아님). 그런데 `ClovaVoiceClient`가 **API 라우터에
연결돼 있지 않아** 프론트가 "발음 듣기"를 눌러도 칠 데가 없습니다. 이걸 붙이는 게 다음 작업입니다.

붙일 때 같이 넣을 것:
1. **캐시** — `권리` 같은 단어는 프로젝트마다 반복된다. 매번 재합성하면 돈이 그대로 나간다.
2. **레이트 리밋 유료 등급에 포함** — `utils/rate_limit.py`의 expensive 경로 목록에 추가.
3. **`run_in_threadpool` 오프로드** — `requests.post`가 블로킹이다. 이 프로젝트는 같은 실수를
   한 번 겪었고 `tests/test_blocking_offload.py`가 그 회귀를 막고 있다. 같은 방식으로 고정할 것.
4. **단가** — `utils/usage_tracker.py:23`의 `CLOVA_VOICE_PRICE_PER_CHAR_KRW`가 아직 `None`이라
   비용이 TBD로만 찍힌다. **NCP 콘솔 단가를 사용자에게 물어봐야 한다**(추측해서 넣지 말 것).

**미해결 판단 1건 — 철자 vs 발음 중 무엇을 합성할지.** 실측으로 Clova Voice는 두 입력을 다르게
읽는다(`권리` 0.96초 vs `궐리` 0.89초, 4개 단어 전부 파일이 다름). 비교용 MP3 8개를 만들어
사용자에게 들어보라고 넘긴 상태다. **답을 아직 못 받았다.**
- 잠정 권고: **철자를 보낸다.** Clova Voice는 한국어 TTS라 표준 발음법을 자체 적용하므로,
  `여칼`을 보내면 음운 규칙이 이중 적용될 위험이 있다. 다만 귀로 확인해야 확정된다.
- 사용자 답이 없으면 철자 기준으로 구현하되, 발음 문자열도 받을 수 있게 파라미터로 열어둘 것.

이어서 **전체 테스트를 자율로** 진행. 실서버 전체 체인은 2026-08-06에 제로 녹음으로 이미 한 번
통과했다(아래 "실사용 검증" 참고) — TTS를 붙인 뒤 같은 방식으로 다시 돌리면 된다.
...
| 최신 커밋 | `2c9d356` (PR #31 머지). 열린 PR 없음 |
| 테스트 | **248건 통과** |
| 엔드포인트 | 12개 (`/` + `/api/*` 11개) |
...
| `CLOVA_VOICE_CLIENT_ID` / `_SECRET` | ✅ **8/06 발급** | TTS. 실호출 성공 확인 |
| `STDICT_API_KEY` | ✅ | 장단음 |
...
⚠️ **Clova Voice 시크릿이 채팅 평문으로 오갔다.** 시연 후 NCP 콘솔에서 재발급 권장.
