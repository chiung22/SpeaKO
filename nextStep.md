...
4. **발음 팁 구조화** — `practice_tips`를 `[{key, title, description}]`으로. `key`는 서버가 정한 목록(`consonant`/`ending`/`intonation`/`speed`/`volume`/`general`) 중 하나라 프론트가 아이콘을 안정적으로 매핑할 수 있다.
   - **옛 캐시 하위호환 포함** — `evaluations.feedback`에 문자열 리스트로 저장된 지난 피드백을 읽을 때 새 형식으로 변환한다. 안 하면 지난 평가 화면만 깨진다.

### PM에게 확인받을 것 (구현은 해뒀음)

- **발음 팁 `key` 목록**이 피그마 아이콘 3종과 맞는지 (`consonant`/`ending`/`intonation`이 피그마의 "명백한 자음 발음"/"끝소리 주의"/"강세와 억양"에 대응하도록 잡음).
- **설명 문구 톤** — 서버가 규칙 판정으로 생성하는 것으로 결정했음. 피그마 시안 문구는 내용이 틀려서 쓰지 않았고, 정확한 문구로 대체했음.
