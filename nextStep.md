# 🔖 작업 재개 지점 (2026-08-22 기준) — 프로젝트 종료

> 시연(2026-08-21) 완료. 8/19 연동 실증에서 전 기능 통과, 8/22 사용자 최종 확인
> "배포 서버에서 모든 동작 확인". **추가 개발 없음.**
>
> **예약된 종료:** EC2 크론 `0 15 25 8 *` (= 2026-08-26 00:00 KST)에
> `~/shutdown-apis.sh`가 AI 컨테이너만 중지(+자동재시작 해제)하고(스프링은 유지)
> 크론에서 스스로 제거된다. 로그: `~/api-shutdown.log`.
> 취소하려면: `crontab -e`에서 해당 줄 삭제. 되살리려면: 컨테이너 `docker start speako-ai`,
> 스프링은 계속 떠 있음.
>
> EC2 인스턴스 자체는 내리지 않았다 — 과금을 멈추려면 AWS 콘솔에서 인스턴스 중지 필요.
...
완료된 라운드: `bdf52ce`(문장형 마무리 제거) → `db11e47`(질문형 도입 반복 6/14→0/14,
시작 인사 제거, 빈 장 동일 대본 분화 — 이미지형 재생성으로 실측 검증됨). 다음 후보:

- **스크린샷 UI 텍스트 오인** — 앱 화면 캡처가 든 슬라이드에서 비전이 화면 속 예문
  ("해당 판례는 이번 계약에…")을 슬라이드 내용으로 읽어 대본이 생뚱맞아짐
  (110번 10장 실측). 비전 프롬프트에 "UI 예문/버튼 문구는 내용이 아니다" 지시 검토
- **장면 이해 2트랙 비전** (반나절, PPT 주인 피드백 2026-08-19) — "진순"이 발표자
  닉네임인 걸 시스템이 모름. 비전이 글자 옮겨적기 전용이라(과거 묘사 환각 사고로 묶어둠)
  `인물+말풍선=자기소개 장` 같은 장면 판단이 불가. 방향: ① 글자 전사(현행) +
  ② `[화면 묘사]` 라벨 분리된 한 줄 장면 설명 → 생성기는 ②를 "장의 종류" 힌트로만
  사용. 라벨 분리 가드 테스트 필수 (image_text_extractor.py의 strip_vision_refusal 참고)
- **CLOVA OCR 폴백** (반나절, 2026-08-19 실험으로 확정) — **HCX-005 비전은 큰 일반폰트도
  못 읽는다.** 실측: "반갑습니다! 진순 입니다:)"(608×189)에서 노란 박스 친 "진순"만 읽고
  나머지 무시, "김로로 (22세, 여성)"(253×54)은 "텍스트 없음". **2·3배 확대 무효,
  프롬프트 4종(현행/단순/줄단위/OCR역할) 전부 무효** — 줄단위 지시엔 개인정보보호법
  위반이라며 거부하기까지. 같은 이미지를 Claude 비전은 완벽히 읽음 = HCX 역량 한계 확정.
  ⚠️ 업스케일·프롬프트 개선안은 이미 실험했으니 재시도 말 것. 답은 전용 OCR:
  HCX 비전이 빈 값을 준 이미지만 CLOVA OCR(NCP 콘솔 발급)로 재시도하는 폴백.
  firecrawl은 웹 크롤링 SaaS라 해당 없음(2026-08-19 검토).
- **핵심 문구 반복** — "배려심과 협동심" 6장 중 5장(102번). 교차 맥락 없는 구조적 한계.
  이웃 장 "내용"을 넘기면 역효과였음(generator.py 주석 참고)
